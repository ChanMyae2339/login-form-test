...
const Table = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            await axiosPrivate(`https://backend-test-gilt-eta.vercel.app/api/users?page=${page}&limit=${limit}${filterQuery}${searchKeyword ? `&search=${searchKeyword}` : ''}`, {
...
const ReactTable = ({ data, columns }) => {
    ...
    useEffect(() => {
        const updateColumnsVisibility = () => {
            ...
            // console.log(visible); // Testing the visible columns

            // console.log(hidden); //Tesinting the hidden columns
        ...
        () => ({
            ...
            header: ({ table }) => (
                <button
                    ...
                    // className="bg-green-700 text-white rounded px-2 py-1 hover:bg-green-800"
    ...
    >
        <div>{hiddenColumns.map((col) => {
            ...
            const value =
                key && data ? data[key] : "";
            return (
                <div key={key} className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex flex-col bg-gradient-to-r from-indigo-300 to-sky-300 p-3 rounded-lg border border-gray-100 shadow-sm">
...
const SearchAndFilterComponent = ({
    ...
    return (
        <div className="mt-5  grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-4">
            <form className="relative w-full lg:basis-1/2 flex flex-row items-center gap-4 ml-2" onSubmit={form.handleSubmit(onSubmit)}>

              <div className="relative">
                  <input className="search-input border p-2 rounded w-full md:w-64 outline-none border-blue-300 " {...form.register("text")}
                 placeholder={`${ isLoading ?"Loading" :"Search"}`} autoComplete="off" />



                <button className="btn text-blue-500 font-semibold absolute -mx-8 top-1/3 " type="submit">
            ...
            <div className="flex   gap-3 place-content-end mr-3">
                <button
                    className="relative btn btn-filled-warning font-semibold"
                    onClick={() => setIsSideOverOpen(true)}
                    type="button"
                >
                    <PiFunnelFill className="icon-content" />
